...
The Konductor Baton is a web-based dashboard served by the Konductor MCP Server that provides per-repository visibility into concurrent development activity. Each repository tracked by Konductor gets its own dedicated page at a predictable URL (`/repo/:owner/:repo`). The repo page surfaces real-time collision state, notifications and alerts, user query logs, and open pull requests. Users receive the URL to their repo page when their client connects to Konductor and can request the URL at any time via IDE chat. A separate admin page is planned for future phases.
...
- **Repo Page**: A dedicated web page for a single repository, accessible at `/repo/:owner/:repo`
...
1. WHEN a user navigates to `/repo/:owner/:repo`, THE Baton SHALL render a page displaying the repository summary, notifications, query log, and open PRs sections for that repository
...
2. WHEN displaying the notifications table, THE Baton SHALL show columns for Timestamp, Notification Type (Healthy, Warning, Alerting), State (Solo, Neighbors, Crossroads, Collision Course, Merge Hell), JIRAs (ticket identifiers if known, "unknown" otherwise), Summary, Users, and a Resolve button
3. WHEN displaying the Users column, THE Baton SHALL show each user name as a link to the user's GitHub profile and the branch name as a link to the GitHub branch
...
2. WHEN displaying the query log table, THE Baton SHALL show columns for Timestamp, User, Query Type, and Parameters
...
3. THE repo page URL SHALL follow the pattern `http://<host>:<port>/repo/<owner>/<repo>` where host, port, owner, and repo are derived from the server configuration and session registration
